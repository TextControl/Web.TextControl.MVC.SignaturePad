...
b.HiddenTextField=(function(){function d(e,f,g){this.canvas=e;
...
}Object.defineProperty(d.prototype,"top",{set:function(e){this.elemInput.style.top=e+"px";
},enumerable:true,configurable:true});
Object.defineProperty(d.prototype,"left",{set:function(e){this.elemInput.style.left=e+"px";
},enumerable:true,configurable:true});
d.prototype.compositionstartHandler=function(h){b.inIMEComposition=true;
var i=this.elemInput;
var f=this.owner.caretBounds;
var g=this.owner.canvas;
i.style.left=f.x+g.offsetLeft+"px";
i.style.top=f.y+g.offsetTop+f.height+"px";
i.style.zIndex=1000;
};
d.prototype.compositionendHandler=function(f){b.inIMEComposition=false;
var h=this.elemInput;
h.style.left=null;
h.style.top=null;
h.style.zIndex=null;
if(h.tagName==="DIV"){h.innerHTML="";
}for(var g=0;
g<f.data.length;
++g){b.sendCommand(b.Command.ComponentEvent,b.UserInputEventType.KeyPress,0,0,{key:f.data[g],modifierKeys:0});
}};
...
d.prototype.elemInput_Focus=function(e){if(this.elemInput.tagName==="INPUT"){this.elemInput.value="";
}else{this.elemInput.innerHTML="";
}};
d.prototype.elemInput_Blur=function(e){b.sendCommand(b.Command.ComponentActivationState,this.componentType,b.ActivationState.Deactivated);
};
d.prototype.update=function(f,e){if(f!=null){this.elemInput.value=f;
}if(this.elemInput.selectionStart!=e){this.elemInput.setSelectionRange(e,e);
}};
return d;
})();
return c;
...
var TXTextControl=(function(i,a){var b=i._private=i._private||{};
...
var f={Left:1,Right:2,Middle:16};
var c={IBeam:1,ArrowNE:2,VSplit:3,UpArrow:4,Arrow:5,DownArrow:6,SizeWE:7,SizeNS:8,SizeNWSE:9,SizeNESW:10,SizeAll:11,Cross:12};
var g={MoveAndSizeFrame:1,TableBorderMovingLine:2};
b.cloneTouch=function(j){return{clientX:j.clientX,clientY:j.clientY,screenX:j.screenX,screenY:j.screenY,identifier:j.identifier,hasLeftDeadzone:j.hasLeftDeadzone};
};
var e={touches:{},getItem:function(j){return this.touches["_"+j];
},add:function(k){var j=k.identifier;
this.touches["_"+j]=b.cloneTouch(k);
},removeItem:function(j){var k=this.getItem(j);
delete this.touches["_"+j];
return k;
},update:function(k){for(var j=0;
j<k.length;
++j){this.add(k[j]);
}},count:function(){var k=0,j;
for(j in this.touches){if(this.touches.hasOwnProperty(j)){++k;
}}return k;
},clear:function(){this.touches={};
}};
var h=(function(){var k=250,j=500,m=500;
q.State={Initial:0,OneFinger:1,NoFingerAfterOneFinger:2,ImpendingDoubleClickOrObjectGrip:3,ObjectGrip:4};
q.TouchEventType={Start:0,Move:1,End:2};
function q(r){this.state=q.State.Initial;
this.viewer=r;
this.targetElem=r.canvas;
this.touch=null;
this.timeoutID=null;
}function o(v,t,r,u){var s=document.createEvent("MouseEvents");
s.initMouseEvent(t,true,true,window,1,v.screenX,v.screenY,v.clientX,v.clientY,false,false,false,false,r,null);
u.dispatchEvent(s);
}q.prototype.simulateMouseClick=function(s,r){o(s,"mouseover",r,this.targetElem);
o(s,"mousemove",r,this.targetElem);
o(s,"mousedown",r,this.targetElem);
o(s,"mouseup",r,this.targetElem);
o(s,"mousemove",r,this.targetElem);
o(s,"mouseout",r,this.targetElem);
o(s,"click",r,this.targetElem);
};
q.prototype.simulateMouseDoubleClick=function(s,r){o(s,"mouseover",r,this.targetElem);
o(s,"mousemove",r,this.targetElem);
o(s,"mousedown",r,this.targetElem);
o(s,"mouseup",r,this.targetElem);
o(s,"click",r,this.targetElem);
...
o(s,"mouseout",r,this.targetElem);
o(s,"click",r,this.targetElem);
o(s,"dblclick",r,this.targetElem);
};
q.prototype.reset=function(){if(this.timeoutID){clearTimeout(this.timeoutID);
this.timeoutID=null;
}if(this.touch){o(this.touch,"mouseup",0,this.targetElem);
o(this.touch,"mousemove",0,this.targetElem);
o(this.touch,"mouseout",0,this.targetElem);
this.touch=null;
}this.state=q.State.Initial;
};
function n(){b.ignoreNextTextFocusRequest=true;
this.simulateMouseClick(this.touch,2);
this.timeoutID=this.touch=null;
this.state=q.State.Initial;
}q.prototype.initialState=function(s,r){switch(r){case q.TouchEventType.Start:this.timeoutID=setTimeout(n.bind(this),j);
this.touch=b.cloneTouch(s);
this.state=q.State.OneFinger;
break;
default:this.reset();
}};
function l(){this.timeoutID=this.touch=null;
this.state=q.State.Initial;
}q.prototype.oneFingerState=function(s,r){switch(r){case q.TouchEventType.End:if(this.timeoutID){clearTimeout(this.timeoutID);
}this.timeoutID=setTimeout(l.bind(this),k);
this.touch=b.cloneTouch(s);
this.state=q.State.NoFingerAfterOneFinger;
break;
default:this.reset();
}};
function p(r){r=r||this;
o(r.touch,"mouseover",0,r.targetElem);
o(r.touch,"mousemove",0,r.targetElem);
o(r.touch,"mousedown",0,r.targetElem);
r.timeoutID=r.touch=null;
r.state=q.State.ObjectGrip;
}q.prototype.noFingerAfterOneFingerState=function(s,r){switch(r){case q.TouchEventType.Start:if(this.timeoutID){clearTimeout(this.timeoutID);
}this.timeoutID=setTimeout(p.bind(this),m);
this.touch=b.cloneTouch(s);
this.state=q.State.ImpendingDoubleClickOrObjectGrip;
break;
default:this.reset();
}};
q.prototype.impendingDoubleClickOrObjectGripState=function(s,r){switch(r){case q.TouchEventType.End:if(this.timeoutID){clearTimeout(this.timeoutID);
this.timeoutID=null;
}this.simulateMouseDoubleClick(s,0);
this.state=q.State.Initial;
break;
case q.TouchEventType.Move:if(this.timeoutID){clearTimeout(this.timeoutID);
this.timeoutID=null;
}p(this);
this.touch=b.cloneTouch(s);
break;
...
q.prototype.objectGripState=function(s,r){switch(r){case q.TouchEventType.Move:o(s,"mousemove",0,this.targetElem);
this.touch=b.cloneTouch(s);
break;
case q.TouchEventType.End:o(s,"mouseup",0,this.targetElem);
o(s,"mousemove",0,this.targetElem);
o(s,"mouseout",0,this.targetElem);
this.touch=b.cloneTouch(s);
this.state=q.State.Initial;
break;
default:this.reset();
}};
q.prototype.processTouch=function(s,r){switch(this.state){case q.State.Initial:this.initialState(s,r);
break;
case q.State.OneFinger:this.oneFingerState(s,r);
break;
case q.State.NoFingerAfterOneFinger:this.noFingerAfterOneFingerState(s,r);
break;
case q.State.ImpendingDoubleClickOrObjectGrip:this.impendingDoubleClickOrObjectGripState(s,r);
break;
case q.State.ObjectGrip:this.objectGripState(s,r);
break;
}};
return q;
})();
...
b.ComponentViewer=(function(){var o="1px dashed black",p="lightgray";
function j(q,r,t,s){this.canvas=q;
this.context=q.getContext("2d");
this.mouseButtons=0;
this.imgBuffer=null;
this.shapeDIV=null;
...
this.touchStateMachine=b.isTouchDevice?new h(this):null;
this.dialogNestingDepth=s||0;
this.caret=new b.Caret(q);
if((r==b.ComponentType.TextView)||(r==b.ComponentType.TxDialog)){this.hiddenTextField=new b.HiddenTextField(q,r,this);
}this.componentType=r;
this.dialogType=t||b.TXDialogType.Unknown;
...
q.addEventListener("mousedown",this.canvas_MouseDown.bind(this));
q.addEventListener("mouseup",this.canvas_MouseUp.bind(this));
q.addEventListener("click",this.canvas_Click.bind(this));
this.shapeDIV.addEventListener("mouseup",this.canvas_MouseUp.bind(this));
q.addEventListener("mousemove",this.canvas_MouseMove.bind(this));
q.addEventListener("dblclick",this.canvas_DoubleClick.bind(this));
q.addEventListener("DOMMouseScroll",this.canvas_MouseWheel.bind(this));
q.addEventListener("mousewheel",this.canvas_MouseWheel.bind(this));
q.addEventListener("contextmenu",function(u){u.preventDefault();
});
if(b.isTouchDevice){q.addEventListener("touchstart",this.canvas_TouchStart.bind(this));
q.addEventListener("touchend",this.canvas_TouchEnd.bind(this));
q.addEventListener("touchcancel",this.canvas_TouchCancel.bind(this));
q.addEventListener("touchleave",this.canvas_TouchEnd.bind(this));
q.addEventListener("touchmove",this.canvas_TouchMove.bind(this));
...
}}Object.defineProperty(j.prototype,"width",{get:function(){return this.canvas.width;
},set:function(q){this.canvas.width=q;
},enumerable:true,configurable:true});
Object.defineProperty(j.prototype,"height",{get:function(){return this.canvas.height;
},set:function(q){this.canvas.height=q;
},enumerable:true,configurable:true});
Object.defineProperty(j.prototype,"left",{get:function(){return this.canvas.offsetLeft;
},set:function(q){this.canvas.style.left=q+"px";
},enumerable:true,configurable:true});
Object.defineProperty(j.prototype,"top",{get:function(){return this.canvas.offsetTop;
},set:function(q){this.canvas.style.top=q+"px";
},enumerable:true,configurable:true});
Object.defineProperty(j.prototype,"caretBounds",{get:function(){return this.caret.bounds;
},enumerable:true,configurable:true});
j.prototype.setCaretState=function(q){if(b.isTouchDevice&&!b.inIMEComposition&&q.Visibility&&this.hiddenTextField){this.hiddenTextField.top=q.Location.Y+this.canvas.offsetTop;
}this.caret.setState(q);
};
j.prototype.setImage=function(s,q){var t=this;
q=q||false;
var r=new Image();
r.onload=function(){if(q){t.canvas.width=r.naturalWidth;
t.canvas.height=r.naturalHeight;
}t.context.clearRect(0,0,r.naturalWidth,r.naturalHeight);
t.context.drawImage(r,0,0,r.naturalWidth,r.naturalHeight);
b.sendCommand(b.Command.ImageLoaded,t.componentType,t.dialogType,t.dialogNestingDepth);
};
r.src=s;
};
j.prototype.updateImageRegions=function(s){var r=this,t=0,q=s.length;
s.forEach(function(v){var u=new Image();
u.onload=function(){r.context.clearRect(v.pos.X,v.pos.Y,u.naturalWidth,u.naturalHeight);
r.context.drawImage(u,v.pos.X,v.pos.Y,u.naturalWidth,u.naturalHeight);
if(q==++t){b.sendCommand(b.Command.ImageLoaded,r.componentType,r.dialogType,r.dialogNestingDepth);
}};
u.src=v.imageDataURL;
});
};
j.prototype.moveAndUpdateContent=function(r,t){var u=this,s=0,q=t.length;
if(!this.canvas.width||!this.canvas.height){return;
}this.context.drawImage(this.canvas,r.X,r.Y,this.canvas.width,this.canvas.height);
t.forEach(function(w){var v=new Image();
v.onload=function(){u.context.clearRect(w.pos.X,w.pos.Y,v.naturalWidth,v.naturalHeight);
u.context.drawImage(v,w.pos.X,w.pos.Y,v.naturalWidth,v.naturalHeight);
if(q==++s){b.sendCommand(b.Command.ImageLoaded,u.componentType,u.dialogType,u.dialogNestingDepth);
}};
v.src=w.imageDataURL;
});
};
j.prototype.show=function(){this.canvas.style.display="block";
};
j.prototype.hide=function(){this.canvas.style.display="none";
};
j.prototype.showCaret=function(){this.caret.show();
};
j.prototype.hideCaret=function(){this.caret.hide();
};
j.prototype.refreshCaretLocation=function(){this.caret.refreshLocation();
};
j.prototype.bufferContent=function(){if(!this.canvas.width||!this.canvas.height){this.imgBuffer=null;
return;
}this.imgBuffer=this.context.getImageData(0,0,this.canvas.width,this.canvas.height);
};
j.prototype.drawBufferedContent=function(){if(!this.imgBuffer){return;
}this.context.putImageData(this.imgBuffer,0,0);
delete this.imgBuffer;
};
j.prototype.canvas_MouseDown=function(r){if(!b.mouseOwner){b.mouseOwner=this;
}else{if(b.mouseOwner!=this){b.mouseOwner.canvas_MouseDown(r);
return;
}}var s=this.getMousePos(r),q={mousePos:{X:Math.round(s.x),Y:Math.round(s.y)},modifierKeys:b.modifierKeys,buttons:this.mouseButtonToMouseButtonFlag(r.which)};
b.sendCommand(b.Command.ComponentEvent,b.UserInputEventType.MouseDown,this.componentType,0,q);
if(r.which==1){this.mouseButtons|=f.Left;
}if(r.which==2){this.mouseButtons|=f.Middle;
}if(r.which==3){this.mouseButtons|=f.Right;
}r.preventDefault();
};
j.prototype.canvas_MouseUp=function(r){if(b.mouseOwner&&(b.mouseOwner!=this)){b.mouseOwner.canvas_MouseUp(r);
return;
}b.lastMouseUpEvent=r;
var s=this.getMousePos(r),q={mousePos:{X:Math.round(s.x),Y:Math.round(s.y)},modifierKeys:b.modifierKeys,buttons:this.mouseButtonToMouseButtonFlag(r.which)};
b.sendCommand(b.Command.ComponentEvent,b.UserInputEventType.MouseUp,this.componentType,0,q);
if(r.which==1){this.mouseButtons&=~f.Left;
}if(r.which==2){this.mouseButtons&=~f.Middle;
}if(r.which==3){this.mouseButtons&=~f.Right;
}this.canvasClientBoundRect=null;
if(r.which==1){this.hideShape();
}r.preventDefault();
};
j.prototype.canvas_Click=function(q){this.focus();
};
j.prototype.canvas_MouseMove=function(r){if(b.mouseOwner&&(b.mouseOwner!=this)){b.mouseOwner.canvas_MouseMove(r);
return;
}var s=this.getMousePos(r),q={mousePos:{X:Math.round(s.x),Y:Math.round(s.y)},modifierKeys:b.modifierKeys,buttons:this.mouseButtons};
b.sendCommand(b.Command.ComponentEvent,b.UserInputEventType.MouseMove,this.componentType,0,q);
r.preventDefault();
};
j.prototype.canvas_DoubleClick=function(r){var s=this.getMousePos(r),q={mousePos:{X:Math.round(s.x),Y:Math.round(s.y)},modifierKeys:b.modifierKeys,};
b.sendCommand(b.Command.ComponentEvent,b.UserInputEventType.MouseDoubleClick,this.componentType,0,q);
r.preventDefault();
};
j.prototype.canvas_TouchStart=function(q){b.hideAllOtherMenus();
b.fileMenu.hide();
var r=q.changedTouches;
if((r.length==1)&&(e.count()==0)){this.canvasClientBoundRect=this.canvas.getBoundingClientRect();
this.touchStateMachine.processTouch(r[0],h.TouchEventType.Start);
}else{this.touchStateMachine.reset();
}e.update(r);
};
j.prototype.canvas_TouchDragTextView=function(r){var s=e.getItem(r.identifier);
if(this.componentType==b.ComponentType.TextView){var q={X:s.clientX-r.clientX,Y:s.clientY-r.clientY};
q=b.pointPixelToTwips(q,b.zoom);
b.scrollDelta(q);
}};
j.prototype.canvas_TouchPinchTextView=function(C){b.isPinchZooming=true;
var A=C[0],D=e.getItem(A.identifier);
var B,E;
if(C.length==2){B=C[1],E=e.getItem(B.identifier);
}else{for(var z in e.touches){if(z!=("_"+A.identifier)){E=B=e.touches[z];
break;
}}}var x={X:D.clientX,Y:D.clientY},y={X:E.clientX,Y:E.clientY},t=k(y,x),r=m(x,y);
x={X:A.clientX,Y:A.clientY},y={X:B.clientX,Y:B.clientY};
var s=k(y,x),q=m(x,y);
var w={X:Math.round(r.X-q.X),Y:Math.round(r.Y-q.Y)};
var u=Math.sqrt(w.X*w.X+w.Y*w.Y);
if(u>0){w=b.pointPixelToTwips(w,b.zoom);
b.scrollDelta(w);
}var v=s/t;
b.zoom=Math.round(b.zoom*v);
b.sendCommand(b.Command.Zoom,b.zoom);
};
function n(t){var q=t.changedTouches;
var v=[];
for(var u=0;
u<q.length;
++u){var r=q[u];
var w=e.getItem(r.identifier);
if(w.hasLeftDeadzone){r.hasLeftDeadzone=true;
v.push(r);
}else{var s=l(w,r);
if(s>=d){r.hasLeftDeadzone=true;
v.push(r);
}}}return v;
}j.prototype.canvas_TouchMove=function(q){if(!e.count()){if(this.touchStateMachine.state!=h.State.Initial){this.touchStateMachine.reset();
}return;
}var r=n(q);
if(r.length){switch(this.touchStateMachine.state){case h.State.ObjectGrip:case h.State.ImpendingDoubleClickOrObjectGrip:this.touchStateMachine.processTouch(r[0],h.TouchEventType.Move);
break;
default:switch(this.componentType){case b.ComponentType.TextView:this.canvas_TouchMoveTextView(r);
break;
}this.touchStateMachine.reset();
}e.update(r);
}if(q.cancelable){q.preventDefault();
}};
j.prototype.canvas_TouchMoveTextView=function(q){switch(e.count()){case 1:this.canvas_TouchDragTextView(q[0]);
break;
case 2:this.canvas_TouchPinchTextView(q);
break;
}};
j.prototype.canvas_TouchEnd=function(q){b.isPinchZooming=false;
var t=q.changedTouches;
if(t.length==1){this.touchStateMachine.processTouch(t[0],h.TouchEventType.End);
}for(var r=0;
r<t.length;
++r){var s=t[r];
e.removeItem(s.identifier);
}};
j.prototype.canvas_TouchCancel=function(q){var t=q.changedTouches;
if(t.length==1){this.touchStateMachine.processTouch(t[0],h.TouchEventType.End);
}for(var r=0;
r<t.length;
++r){var s=t[r];
...
function k(s,t){var q=t.X-s.X;
var r=t.Y-s.Y;
return Math.sqrt((q*q)+(r*r));
}function l(s,t){var q={X:s.clientX,Y:s.clientY},r={X:t.clientX,Y:t.clientY};
return k(r,q);
}function m(q,r){return{X:(q.X+r.X)/2,Y:(q.Y+r.Y)/2};
}j.prototype.canvas_MouseWheel=function(s){var r=(s.wheelDelta||-(s.detail*40)),t=this.getMousePos(s),q={mousePos:{X:Math.round(t.x),Y:Math.round(t.y)},modifierKeys:b.modifierKeys,delta:r};
b.sendCommand(b.Command.ComponentEvent,b.UserInputEventType.MouseWheel,this.componentType,0,q);
s.preventDefault();
};
j.prototype.cursorChanged=function(r){var s=r.detail;
var q;
if(s.target!=this.componentType){return;
}if(s.dialogType!=this.dialogType){return;
}if(this.mouseButtons&f.Left){return;
}this.cursorType=s.cursorType;
switch(s.cursorType){case c.IBeam:this.canvas.style.cursor="text";
break;
case c.ArrowNE:if((b.BrowserDetect.browser==="Explorer")||(b.BrowserDetect.browser==="Other")||(b.BrowserDetect.browser==="Edge")){if(typeof TXWebResources!=="undefined"){q="url("+TXWebResources.m_srcCursors_markline_cur+"), default";
}else{q="url("+b.getTXFileDir()+"cursors/markline.cur), default";
}}else{q="url(data:image/png;base64,) 32 0, default";
}this.canvas.style.cursor=q;
break;
case c.VSplit:this.canvas.style.cursor="col-resize";
break;
case c.UpArrow:this.canvas.style.cursor="pointer";
break;
case c.Arrow:this.canvas.style.cursor="default";
break;
case c.DownArrow:if((b.BrowserDetect.browser==="Explorer")||(b.BrowserDetect.browser==="Other")||(b.BrowserDetect.browser==="Edge")){if(typeof TXWebResources!=="undefined"){q="url("+TXWebResources.m_srcCursors_tablecol_cur+"), default";
}else{q="url("+b.getTXFileDir()+"cursors/tablecol.cur), default";
}}else{q="url(data:image/png;base64,) 23 11, default";
}this.canvas.style.cursor=q;
break;
case c.SizeWE:this.canvas.style.cursor="ew-resize";
break;
case c.SizeNS:this.canvas.style.cursor="ns-resize";
break;
case c.SizeNWSE:this.canvas.style.cursor="nwse-resize";
break;
case c.SizeNESW:this.canvas.style.cursor="nesw-resize";
break;
case c.SizeAll:this.canvas.style.cursor="move";
break;
case c.Cross:this.canvas.style.cursor="crosshair";
break;
default:this.canvas.style.cursor="default";
break;
}};
j.prototype.shapeBlink=function(){if(this.shapeIsBlack){this.shapeDIV.style.borderColor=p;
this.shapeIsBlack=false;
}else{this.shapeDIV.style.borderColor="black";
this.shapeIsBlack=true;
}};
j.prototype.hideShape=function(){if(this.shapeBlinkTimer){clearTimeout(this.shapeBlinkTimer);
...
j.prototype.drawShape=function(q){if(!(this.mouseButtons&f.Left)){return;
}var r=q.detail;
if(!this.shapeBlinkTimer){this.shapeBlinkTimer=setInterval(this.shapeBlink.bind(this),500);
switch(r.Kind){case g.TableBorderMovingLine:this.shapeDIV.style.border="none";
this.shapeDIV.style.borderLeft=o;
break;
case g.MoveAndSizeFrame:this.shapeDIV.style.border=o;
break;
}this.shapeDIV.style.opacity=1;
}var s=r.Location.X+this.left,t=r.Location.Y+this.top;
this.shapeDIV.style.top=t+"px";
this.shapeDIV.style.left=s+"px";
this.shapeDIV.style.height=r.Size.Height+"px";
this.shapeDIV.style.width=r.Size.Width+"px";
};
j.prototype.focus=function(){if(b.ignoreNextTextFocusRequest){b.ignoreNextTextFocusRequest=false;
return;
...
j.prototype.blur=function(){if(this.hiddenTextField){this.hiddenTextField.blur();
}};
j.prototype.getMousePos=function(q){var r;
if(b.isTouchDevice&&this.canvasClientBoundRect){r=this.canvasClientBoundRect;
}else{r=this.canvas.getBoundingClientRect();
}return{x:q.clientX-r.left,y:q.clientY-r.top};
};
j.prototype.mouseButtonToMouseButtonFlag=function(q){switch(q){case 1:return f.Left;
case 2:return f.Middle;
...
var TXTextControl=(function(I,a){var b=I._private=I._private||{};
var r,q,p,o;
var n={scrollEnabled:false,deltaY:0,maxDelta:0,touchInProgress:false,touchPrev:null,reset:function(){this.scrollEnabled=false;
...
var v=(function(){function K(L){L=L||"MERGEFIELD";
this.Name=L;
this.Text="«"+L+"»";
this.TextFormat=0;
this.TextBefore="";
...
b.attachMailingsTabEventHandlers=function(){q=a("#insertSpecialFieldDropDown").menu().hide();
o=a("#insertMergeBlockDropDown").menu().hide();
p=a("#insertMergeFieldDropDown").menu().hide();
q.on("menuselect",C);
o.on("menuselect",y);
p.on("menuselect",A);
document.addEventListener("dbSchemeReceived",f);
document.addEventListener("fieldDisplayModeChanged",h);
document.addEventListener("fieldDataReceived",g);
document.addEventListener("masterTableSelected",s);
document.addEventListener("mergeFieldHighlightingChanged",w);
document.addEventListener("mergeBlockHighlightingChanged",t);
document.addEventListener("txItemReceived",J);
...
var K=function(){b.sendCommand(b.Command.FinishAndMerge);
};
a("#btnFinishAndMerge").click(function(){if(b.documentDirty){b.fileSaveQuestion(K);
}else{K();
}});
var P=function(R){var Q=(R.wheelDelta||-(R.detail*40))/4;
H(Q);
};
var O=function(Q){n.touchInProgress=(Q.touches.length==1);
if(n.touchInProgress){n.touchPrev=b.cloneTouch(Q.touches[0]);
}};
var M=function(Q){n.touchInProgress=false;
n.touchPrev=null;
};
var N=function(R){if(!n.touchInProgress){return;
}if(R.touches.length!=1){n.touchInProgress=false;
return;
}var Q=R.changedTouches[0].clientY-n.touchPrev.clientY;
n.touchPrev=b.cloneTouch(R.changedTouches[0]);
H(Q);
if(R.cancelable){R.preventDefault();
}};
var L=document.getElementById("insertMergeFieldDropDown");
L.addEventListener("mousewheel",P);
L.addEventListener("DOMMouseScroll",P);
if(b.isTouchDevice){L.addEventListener("touchstart",O);
L.addEventListener("touchend",M);
L.addEventListener("touchcancel",M);
L.addEventListener("touchmove",N);
L.style.transition="";
}a("#drpDnBtnSelectMasterTable").click(D);
a("#drpDnBtnInsertMergeField").click(z);
a("#drpDnBtnInsertSpecialField").click(B);
a("#drpDnBtnInsertMergeBlock").click(x);
};
function D(K){b.fileMenu.hide();
b.hideAllMenus_mailingsTab();
if(!r){return;
}r.show().position({my:"left top",at:"left bottom",of:K.currentTarget});
b.limitMenuHeight("selectMasterTableDropDown");
a(document).one("click",function(){r.hide();
});
return false;
}function F(){n.reset();
var K=document.getElementById("insertMergeFieldDropDown");
K.style.transform="";
}function e(){var L=document.getElementById("insertMergeFieldDropDown");
var K=L.clientHeight;
var M=b.clientSize.Height-b.RibbonBarHeight-b.StatusBarHeight;
if(K>M){n.scrollEnabled=true;
n.maxDelta=-(K-M);
}else{n.reset();
}}function H(K){if(!n.scrollEnabled){return;
}var L=document.getElementById("insertMergeFieldDropDown");
n.deltaY=Math.min(Math.max((n.deltaY+K),n.maxDelta),0);
L.style.transform="translate(0px, "+n.deltaY+"px)";
}function z(L){b.fileMenu.hide();
b.hideAllMenus_mailingsTab();
F();
b.sendCommand(b.Command.GetItem,b.ItemType.MergeBlock);
var K=document.getElementById("insertMergeFieldDropDown");
b.calcMenuWidths(K);
p.show().position({my:"left top",at:"left bottom",of:L.currentTarget});
e();
a(document).on("click",function(M){if(!(b.isInsideElementWithID(M.target,"insertMergeFieldDropDown")&&b.isPartOfMenuItemWithSubMenu(M.target))){p.hide();
F();
a(document).off(M);
}});
return false;
}function B(L){b.fileMenu.hide();
b.hideAllMenus_mailingsTab();
var K=document.getElementById("insertSpecialFieldDropDown");
b.calcMenuWidths(K);
q.show().position({my:"left top",at:"left bottom",of:L.currentTarget});
b.limitMenuHeight("insertSpecialFieldDropDown");
a(document).one("click",function(){q.hide();
});
return false;
}function C(L,M){var K;
switch(M.item.context.id){case"mnuItemInsertIFField":K=new b.IfFieldDialog();
break;
case"mnuItemInsertInclTxtField":K=new b.IncludeTextFieldDialog();
break;
case"mnuItemInsertDateField":K=new b.DateFieldDialog();
break;
case"mnuItemInsertNextField":b.sendCommand(b.Command.InsertItem,b.ItemType.FieldAdapter,b.FieldType.NextField);
break;
case"mnuItemInsertNextIfField":K=new b.NextIfFieldDialog();
break;
}if(K){K.showModal();
}}function x(L){b.fileMenu.hide();
b.hideAllMenus_mailingsTab();
if(!o){return;
}b.sendCommand(b.Command.GetItem,b.ItemType.MergeBlock);
var K=document.getElementById("insertMergeBlockDropDown");
b.calcMenuWidths(K);
o.show().position({my:"left top",at:"left bottom",of:L.currentTarget});
b.limitMenuHeight("insertMergeBlockDropDown");
a(document).one("click",function(){o.hide();
});
return false;
}function E(L,N){var K=N.item.context;
var M=k(K);
if(!M||(M.length==0)){return;
}b.selectMasterTable(M);
}function k(K){if(!K||!K.hasAttribute("data-table-name")){return"";
}return K.attributes["data-table-name"].value;
}function j(K){var O="";
if(!K||(!K.hasAttribute("data-column-name")&&!K.hasAttribute("data-child-table-name"))){return"";
}var L=K.parentElement;
while(L&&(L.nodeName.toLowerCase()!=="li")){L=L.parentElement;
}var M="";
if(K.hasAttribute("data-column-name")){M=K.attributes["data-column-name"].value;
}else{if(K.hasAttribute("data-child-table-name")){M=K.attributes["data-child-table-name"].value;
}}var N=j(L);
if(N.length){O=N+"."+M;
}else{O=M;
}return O;
}b.insertMergeField=function(L){var K={Name:L,Text:"«"+L+"»"};
b.sendCommand(b.Command.InsertItem,b.ItemType.FieldAdapter,b.FieldType.MergeField,0,K);
};
b.insertMergeBlock=function(K){K=JSON.stringify(K);
b.sendCommand(b.Command.InsertItem,b.ItemType.MergeBlock,0,0,K);
};
function A(N,Q){var L=Q.item.context;
if(L.querySelectorAll("ul").length){return;
}var P=j(L)||L.id;
if(!P||(P.length==0)){return;
}switch(P){case"mnuItemInsertMergeField":var O=new v();
var M=new b.MergeFieldDialog(O);
M.showModal();
break;
case"mnuItemHighlightMergeFields":var K=a("#tglBtnHighlightMergeFields");
if(K.hasClass("ribbon-button-selected")){b.sendCommand(b.Command.HighlightMergeFields,0);
}else{b.sendCommand(b.Command.HighlightMergeFields,1);
}break;
default:b.insertMergeField(P);
}}function y(M,O){var L=O.item.context;
var N=k(L)||L.id;
if(!N||(N.length==0)){return;
}switch(N){case"mnuItemInsertMergeBlock":b.insertMergeBlock();
break;
case"mnuItemHighlightMergeBlocks":var K=a("#tglBtnHighlightMergeBlocks");
if(K.hasClass("ribbon-button-selected")){b.sendCommand(b.Command.HighlightMergeBlocks,0);
}else{b.sendCommand(b.Command.HighlightMergeBlocks,1);
}break;
default:b.insertMergeBlock(N);
}}function f(K){var Q=K.detail;
var N=a("#selectMasterTableDropDown");
var M=a("#insertMergeFieldDropDown");
var L=a("#insertMergeBlockDropDown");
N.empty();
...
var O=[];
O.push(document.getElementById("mnuItemInsertMergeBlock"));
O.push(document.getElementById("mnuItemHighlightMergeBlocks"));
O.forEach(function(R){L[0].removeChild(R);
});
var P=[];
P.push(document.getElementById("mnuItemInsertMergeField"));
P.push(document.getElementById("mnuItemHighlightMergeFields"));
P.forEach(function(R){M[0].removeChild(R);
});
M.empty();
L.empty();
b.dbScheme.Tables.forEach(function(R){d(M,b.dbScheme,R,true,false);
d(N,b.dbScheme,R,false,true);
d(L,b.dbScheme,R,false,false);
});
O.forEach(function(R){L[0].appendChild(R);
});
P.forEach(function(R){M[0].appendChild(R);
});
if(!r){r=N.menu().hide();
r.on("menuselect",E);
}else{N.menu("refresh");
...
L.menu("refresh");
b.calcMenuWidths(N[0]);
if(!Q.suppressMessage){b.showMsgDiv(b.variousMessages.databaseLoadedMessage.text,true);
}}function h(K){var L=K.detail.fieldDisplayMode;
switch(L){case b.FieldDisplayMode.ShowFieldText:a("#tglBtnShowFieldCodes").removeClass("ribbon-button-selected");
...
break;
}}function g(K){b.currentFieldData.fieldAdapter=K.detail.field;
b.currentFieldData.mergeFieldType=K.detail.fieldType;
...
}}function s(M){var N=M.detail.tableName;
b.masterTableName=N;
var L=a("#selectMasterTableDropDown div.ribbon-button");
L.removeClass("ribbon-button-selected");
var K=a('#selectMasterTableDropDown li[data-table-name="'+N.toLowerCase()+'"] div.ribbon-button');
K.addClass("ribbon-button-selected");
}function w(K){var L=K.detail.highlight;
if(L){a("#tglBtnHighlightMergeFields").addClass("ribbon-button-selected");
}else{a("#tglBtnHighlightMergeFields").removeClass("ribbon-button-selected");
}}function t(K){var L=K.detail.highlight;
if(L){a("#tglBtnHighlightMergeBlocks").addClass("ribbon-button-selected");
}else{a("#tglBtnHighlightMergeBlocks").removeClass("ribbon-button-selected");
}}function J(K){var M=K.detail.itemType;
var L=K.detail.item;
switch(M){case b.ItemType.MergeBlock:u(L);
break;
}}function u(K){if(!b.dbScheme){return;
}G();
if(K==null){m(b.masterTableName);
}else{m(K.Name);
}}function m(S){var P;
S=S.toLowerCase();
var O=document.querySelector('#insertMergeFieldDropDown > li[data-table-name="'+S+'"] > a[role=menuitem]');
var K=document.querySelectorAll("#insertMergeBlockDropDown > li[data-table-name] > a[role=menuitem]");
if(O){O.classList.add("merge-highlight");
}var R=b.dbScheme.Tables.find(function(T){return T.TableName.toLowerCase()===S;
});
if(!R){return;
}var Q=R.ChildRelations;
if(!Q.length){return;
}for(P=0;
P<K.length;
++P){var M=K[P];
var N=M.parentElement;
var L=N.attributes["data-table-name"].value;
if(Q.contains(function(T){return(T.ChildTableName.toLowerCase()===L);
})){M.classList.add("merge-highlight");
}}}function G(){var L=document.querySelectorAll("#insertMergeFieldDropDown > li > a[role=menuitem].merge-highlight");
var K=document.querySelectorAll("#insertMergeBlockDropDown > li > a[role=menuitem].merge-highlight");
for(var M=0;
M<L.length;
++M){L[M].classList.remove("merge-highlight");
}for(M=0;
M<K.length;
++M){K[M].classList.remove("merge-highlight");
}}function d(R,L,V,K,U){var Q=document.createElement("li");
R.append(Q);
Q.setAttribute("data-table-name",V.TableName.toLowerCase());
var M=document.createElement("a");
Q.appendChild(M);
M.setAttribute("href","#");
var O=document.createElement("div");
M.appendChild(O);
O.classList.add("large-menu-item-image-container");
if(U){O.classList.add("ribbon-button");
}var P=document.createElement("img");
O.appendChild(P);
P.setAttribute("src",b.imageResources.large.table);
var N=document.createElement("div");
M.appendChild(N);
N.classList.add("large-menu-item-caption-container");
var S=document.createElement("p");
N.appendChild(S);
S.textContent=V.TableName;
if(K){var T=document.createElement("ul");
Q.appendChild(T);
T.classList.add("dropDownMenu");
c(T,L,V);
}}function c(L,K,M){M.Columns.forEach(function(N){var P=document.createElement("li");
L.appendChild(P);
P.setAttribute("data-column-name",N.ColumnName);
var O=document.createElement("a");
P.appendChild(O);
O.textContent=N.ColumnName;
});
M.ChildRelations.forEach(function(R){var P=document.createElement("li");
L.appendChild(P);
P.setAttribute("data-child-table-name",R.ChildTableName.toLowerCase());
var O=document.createElement("a");
P.appendChild(O);
O.textContent=R.ChildTableName;
var Q=document.createElement("ul");
P.appendChild(Q);
Q.classList.add("dropDownMenu");
var N=K.Tables.find(function(S){return(S.TableName.toLowerCase()===R.ChildTableName.toLowerCase());
});
c(Q,K,N);
});
}b.calcMenuWidths=function(M){var Q=[];
for(var N=0;
N<M.children.length;
++N){var L=M.children[N];
if(L.tagName==="LI"){for(var O=0;
O<L.children.length;
++O){var K=L.children[O];
if(K.tagName==="UL"){Q.push(K);
}}}}for(N=0;
N<Q.length;
++N){var P=Q[N];
b.calcMenuWidths(P);
}M.style.width=i(M)+"px";
};
function l(K){return(K.style.overflowY==="auto")||(K.style.overflowY==="scroll");
}function i(N){var W=0;
var Y;
var K=l(N)?27:10;
for(var S=0;
S<N.children.length;
++S){var Q=N.children[S];
if(!Q.children.length){continue;
}var O=Q.children[0];
if(O.nodeName!=="A"){continue;
}var M=getComputedStyle(O);
var L=O.cloneNode(true);
L.style.fontFamily=M.fontFamily;
...
X.style.fontSize=M.fontSize;
X.style.fontFamily=M.fontFamily;
var Z=document.createElement("TR");
X.appendChild(Z);
for(var T=0;
T<L.childNodes.length;
++T){var V=L.childNodes[T].cloneNode(true);
switch(V.nodeType){case Node.ELEMENT_NODE:if((V.tagName==="DIV")&&(V.classList.contains("menu-item-category-header"))){break;
}Y=document.createElement("TD");
var R=V.querySelectorAll("h1");
for(var U=0;
U<R.length;
++U){R[U].style.fontSize=M.fontSize;
R[U].style.fontWeight="bold";
}Y.appendChild(V);
Z.appendChild(Y);
break;
case Node.TEXT_NODE:Y=document.createElement("TD");
Y.textContent=V.data;
Z.appendChild(Y);
break;
}}var P=document.getElementById("txTemplateDesignerContainer");
P.appendChild(X);
var aa=X.clientWidth+K;
W=Math.max(W,aa);
P.removeChild(X);
}return W;
}b.hideAllMenus_mailingsTab=function(){if(p){p.hide();
}if(r){r.hide();
}if(o){o.hide();
}q.hide();
F();
...
var TXTextControl=(function(aG,a){var b=aG._private=aG._private||{};
aG.version="23.0";
function at(){delete aG._private;
...
var aF={VariousMessages:0,RibbonStrings:1};
var aI={Unknown:0,ServiceNotInstalled:1,ServiceNotRunning:2,CouldNotConnectToService:3};
...
b.sendCommand=function(aN,aP,aQ,aR,aO){if(!b.isSocketConnected()){return;
}var aM={CommandID:parseInt(aN,10),Param1:aP||0,Param2:aQ||0,Param3:aR||0,Data:aO||"",};
var aS=JSON.stringify(aM);
b.socket.send(aS);
};
b.getWebAppRootDir=function(){var aQ="";
var aO="";
var aM="";
if((typeof TXWebResources!=="undefined")&&TXWebResources.hasOwnProperty("webSocketHandlerPath")){aO=TXWebResources.webSocketHandlerPath;
}else{if((typeof TXSettings!=="undefined")){aM=TXSettings.AppVirtualPath;
}}var aP=window.location.pathname.split("/");
aP.pop();
var aN=(aP.length==0)?"/":aP.join("/")+"/";
if(aO){if(aO.slice(-1)!=="/"){aO+="/";
}if(aO[0]==="/"){aQ=window.location.host+aO;
}else{aQ=window.location.host+aN+aO;
}}else{if(aM){aQ=window.location.host+aM;
}else{aQ=window.location.host+aN;
}}return aQ;
};
b.getTXFileDir=function(){var aN=b.getWebAppRootDir();
var aM=b.isSSLConnection?"https://":"http://";
if(typeof TXSettings!=="undefined"){return aM+aN+TXSettings.BasePath;
}else{return aM+aN;
}};
function r(){var aM=(typeof TXSettings!=="undefined")&&TXSettings.IsMVC;
var aO=b.isSSLConnection?"wss://":"ws://";
var aN=b.getWebAppRootDir();
if(aM){return aO+aN+TXSettings.ApiRoute+"TXWebSocket";
}else{return aO+aN+"TXWebSocketHandler.ashx";
}}function f(){var aM=document.querySelectorAll("img[data-web-res-id]");
var aN={};
[].forEach.call(aM,function(aO){aN[aO.getAttribute("data-web-res-id")]=aO;
});
[].forEach.call(TXWebResources.m_imageWebResources,function(aP){var aO=aN[aP.id];
if(aO){aO.src=aP.src;
}});
}function e(){var aM=document.getElementById("txRibbonTabContentContainer");
var aN=aM.querySelectorAll("img[data-orig-src]");
[].forEach.call(aN,function(aO){var aP=aO.getAttribute("data-orig-src");
aO.src=TXSettings.AppVirtualPath+TXSettings.BasePath+aP;
aO.removeAttribute("data-orig-src");
});
}b.changeImageSources=function(){if(typeof TXWebResources!=="undefined"){f();
}else{if(typeof TXSettings!=="undefined"){e();
}}};
b.init=function(){var aO=document.getElementById("mainCanvas");
var aM=document.getElementById("horRulerCanvas");
var aQ=document.getElementById("vertRulerCanvas");
var aP=document.getElementById("statusBarCanvas");
aO.width=window.innerWidth;
aO.height=window.innerHeight;
aM.width=window.innerWidth;
aQ.height=window.innerHeight;
aP.width=window.innerWidth;
b.textViewer=new b.ComponentViewer(aO,b.ComponentType.TextView);
b.horRulerViewer=new b.ComponentViewer(aM,b.ComponentType.HorRuler);
b.vertRulerViewer=new b.ComponentViewer(aQ,b.ComponentType.VertRuler);
b.statusBarViewer=new b.ComponentViewer(aP,b.ComponentType.StatusBar);
...
b.socket.addEventListener("open",aL);
b.socket.addEventListener("message",aK);
b.socket.addEventListener("error",aJ);
b.socket.addEventListener("close",aH);
...
document.addEventListener("pongMessageReceived",y);
document.addEventListener("documentSaved",l);
document.addEventListener("textFieldDataReceived",b.textFieldDataReceivedHandler);
aO.addEventListener("drop",b.handleDrop);
aO.addEventListener("dragenter",function(aR){aR.preventDefault();
aR.stopPropagation();
});
aO.addEventListener("dragover",function(aR){aR.preventDefault();
aR.stopPropagation();
aR.dataTransfer.dropEffect="copy";
});
document.getElementById("ribbonbar").addEventListener("contextmenu",z);
document.getElementById("vertical-scrollbar-wrapper").addEventListener("contextmenu",z);
document.getElementById("horizontal-scrollbar-wrapper").addEventListener("contextmenu",z);
document.getElementById("scroll-button-up").addEventListener("contextmenu",z);
document.getElementById("scroll-button-down").addEventListener("contextmenu",z);
document.getElementById("scroll-button-left").addEventListener("contextmenu",z);
document.getElementById("scroll-button-right").addEventListener("contextmenu",z);
document.addEventListener("mousedown",j);
document.addEventListener("mouseup",k);
document.addEventListener("click",i);
document.addEventListener("mouseout",function(aR){if(!aR.relatedTarget||(aR.relatedTarget.nodeName=="HTML")){t.Left=false;
t.Middle=false;
t.Right=false;
if(b.mouseOwner){b.mouseOwner.canvas_MouseUp(aR);
b.mouseOwner=null;
}}});
b.initContextMenus();
b.createScrollbars();
...
if(!b.isTouchDevice){b.textViewer.focus();
}var aN=document.getElementById("txLoaderGif");
aN.style.opacity=1;
at();
};
a(document).ready(function(){b.init();
});
function c(){var aN=window.MutationObserver||window.WebKitMutationObserver||window.MozMutationObserver;
var aO=new aN(function(aP){aP.forEach(function(aQ){switch(aQ.type){case"attributes":switch(aQ.attributeName){case"style":g();
break;
}break;
}});
});
var aM={attributes:true,childList:false,characterData:false};
aO.observe(document.getElementById("txTemplateDesignerContainer").parentElement,aM);
}function au(){if(typeof TXWebResources=="undefined"){return;
}if(!TXWebResources.hasOwnProperty("connectionID")){return;
}b.sendCommand(b.Command.SetGUID,0,0,0,TXWebResources.connectionID);
}function ab(){if((typeof TXSettings==="undefined")||!TXSettings.IsMVC){return;
...
}function p(aM){if(navigator.userLanguage){aM.culture=navigator.userLanguage;
aM.uiCulture=navigator.browserLanguage;
}else{var aN=navigator.languages?navigator.languages[0]:navigator.language;
if(aN){aM.culture=aM.uiCulture=aN;
}else{console.warn("Unable to detect browser language.");
}}}function q(){var aM={culture:"",uiCulture:""};
p(aM);
if(typeof TXWebResources!=="undefined"){if(TXWebResources.hasOwnProperty("culture")&&TXWebResources.hasOwnProperty("uiCulture")){if(TXWebResources.culture){aM.culture=TXWebResources.culture;
}if(TXWebResources.uiCulture){aM.uiCulture=TXWebResources.uiCulture;
}}}else{if(typeof TXSettings!=="undefined"){if(TXSettings.Culture){aM.culture=TXSettings.Culture;
}if(TXSettings.UICulture){aM.uiCulture=TXSettings.UICulture;
}}}return aM;
}function aL(){au();
var aM=q();
b.sendCommand(b.Command.CreateTextControl,0,0,0,aM);
b.sendCommand(b.Command.IsTouchDevice,b.isTouchDevice?1:0);
ab();
}function w(){try{g();
}catch(aM){}}function g(){var aM=document.getElementById("txTemplateDesignerContainer").parentElement;
var aP,aO;
if(aM.tagName=="BODY"){aP=window.innerWidth;
aO=window.innerHeight;
}else{aP=aM.offsetWidth;
aO=aM.offsetHeight;
}if((aP!=b.clientSize.Width)||(aO!=b.clientSize.Height)){var aN=new CustomEvent("txContainerResized");
document.dispatchEvent(aN);
}}b.blockUserInput=function(){var aM=document.getElementById("txTemplateDesignerContainer");
var aN=document.createElement("DIV");
aN.style.position="absolute";
aN.style.left=aN.style.top="0px";
aN.style.width=aN.style.height="100%";
aN.style.background="rgba(0, 0, 0, 0.1)";
aN.style.zIndex="9998";
aM.appendChild(aN);
};
function m(aN,aO,aM,aP){aN.style.transition="opacity "+(aP/1000)+"s";
setTimeout(function(){aN.style.opacity=0;
setTimeout(function(){aO.removeChild(aN);
},aP);
},aM);
}b.showMsgDiv=function(aP,aO){b.textViewer.hideCaret();
var aM=document.getElementById("txTemplateDesignerContainer");
var aN=document.createElement("DIV");
aN.id="txErrMsgDiv";
aN.classList.add("unselectable");
aN.style.zIndex="9999";
aP=aP.replace(/\r?\n/g,"<br />");
aN.innerHTML=aP;
aM.appendChild(aN);
aN.style.marginTop=-(aN.clientHeight/2)+"px";
aN.style.marginLeft=-(aN.clientWidth/2)+"px";
if(aO){m(aN,aM,1000,1000);
}};
b.showErrMsgDiv=function(){b.showMsgDiv(b.variousMessages.webSocketErrMsg.text);
};
function aE(){b.blockUserInput();
b.showErrMsgDiv();
}function aJ(aM){b.socket=null;
setTimeout(aE,1000);
}function aH(aM){aJ();
}function aK(aM){var aO=""+aM.data;
var aN=JSON.parse(aO);
Z(aN);
}function Z(aO){if(!aO.hasOwnProperty("ID")){console.error("Message has no ID");
return;
}if(!aO.hasOwnProperty("Body")){console.error("Message has no message body");
return;
}switch(aO.ID){case d.FontList:M(aO.Body);
break;
case d.FileList:L(aO.Body);
break;
case d.StyleList:am(aO.Body);
break;
case d.Image:Q(aO.Body);
break;
case d.PrintResult:ac(aO.Body);
break;
case d.FontSizes:N(aO.Body);
break;
case d.InputFormat:S(aO.Body);
break;
case d.DocumentSizeChanged:H(aO.Body);
break;
case d.Zoomed:ar(aO.Body);
break;
case d.InputPositionChanged:T(aO.Body);
break;
case d.TextViewLocation:an(aO.Body);
break;
case d.TextMessage:var aP=b.txDialog?b.txDialog.getActiveViewer():b.textViewer;
b.documentSavedEventHandlers=[];
b.jQueryAlert(aO.Body.caption,aO.Body.message,aP);
break;
case d.DBScheme:E(aO.Body);
break;
case d.SetCursor:ai(aO.Body);
break;
case d.DocumentTargets:I(aO.Body);
break;
case d.Item:V(aO.Body);
break;
case d.DialogResources:F(aO.Body);
break;
case d.SetAppMode:ah(aO.Body);
break;
case d.ShowDialog:ak(aO.Body);
break;
case d.FieldDisplayModeChanged:K(aO.Body);
break;
case d.SetRibbonButtonState:aj(aO.Body);
break;
case d.WaitDialog:ap(aO.Body);
break;
case d.SelectMasterTable:af(aO.Body);
break;
case d.MergeFieldHighlightingChanged:Y(aO.Body);
break;
case d.MergeBlockHighlightingChanged:X(aO.Body);
break;
case d.CaretStateChanged:A(aO.Body);
break;
case d.CloseDialog:B(aO.Body);
break;
case d.ImageRegions:R(aO.Body);
break;
case d.MoveAndUpdate:aa(aO.Body);
break;
case d.ContextMenuData:D(aO.Body);
break;
case d.ConsoleLog:C(aO.Body);
break;
case d.DrawShape:J(aO.Body);
break;
case d.FieldPropertiesDialog:var aM=new CustomEvent("showFieldPropsDialog");
document.dispatchEvent(aM);
break;
case d.HyperlinkClicked:P(aO.Body);
break;
case d.Pong:aM=new CustomEvent("pongMessageReceived");
document.dispatchEvent(aM);
break;
case d.UnknownProtocol:ao();
break;
case d.StringResources:al(aO.Body);
break;
case d.ProofingSettings:ad(aO.Body);
break;
case d.ItemActivationStateChangedEvent:U(aO.Body);
break;
case d.HTMLDialogState:O(aO.Body);
break;
case d.DocumentDirtyChanged:b.documentDirty=aO.Body.documentDirty;
break;
case d.DocumentSaved:var aN=aO.Body.message;
if(aN&&aN.length){b.showMsgDiv(aN,true);
}b.documentDirty=false;
aM=new CustomEvent("documentSaved");
document.dispatchEvent(aM);
break;
case d.EnableSaveDocumentQuestion:b.saveDocumentQuestionEnabled=aO.Body.enabled;
break;
case d.SetTextViewActivationState:av(aO.Body.activationState);
break;
case d.EditMode:b.setEditMode(aO.Body.editMode);
break;
case d.ImageResources:b.imageResources=aO.Body;
break;
case d.MenuContent:W(aO.Body);
break;
case d.WSDisconnectReason:aq(aO.Body);
break;
case d.ServerEvent:ag(aO.Body);
break;
case d.DocumentData:G(aO.Body);
break;
case d.SelectionData:ae(aO.Body);
break;
}}function av(aM){switch(aM){case b.ActivationState.Activated:b.textViewer.focus();
break;
case b.ActivationState.Deactivated:b.textViewer.blur();
break;
}}function Q(aN){var aM=new CustomEvent("imageReceived",{detail:aN});
document.dispatchEvent(aM);
}function M(aN){var aM=new CustomEvent("fontListReceived",{detail:{fontNames:aN.fontNames}});
document.dispatchEvent(aM);
}function h(aN){var aM=new CustomEvent("uiFlagsUpdated",{detail:aN});
document.dispatchEvent(aM);
}function L(aN){var aM=new CustomEvent("fileListReceived",{detail:aN});
document.dispatchEvent(aM);
}function am(aN){var aM=new CustomEvent("styleListReceived",{detail:{styleNames:aN.styleNames}});
document.dispatchEvent(aM);
}function ac(aN){var aM=new CustomEvent("printResultReceived",{detail:aN});
document.dispatchEvent(aM);
}function N(aN){var aM=new CustomEvent("fontSizesReceived",{detail:{fontSizes:aN.fontSizes}});
document.dispatchEvent(aM);
}function S(aN){var aM=new CustomEvent("inputFormatReceived",{detail:aN});
document.dispatchEvent(aM);
}function H(aN){var aM=new CustomEvent("documentSizeChanged",{detail:aN});
document.dispatchEvent(aM);
}function ar(aN){var aM=new CustomEvent("txZoomed",{detail:aN});
document.dispatchEvent(aM);
}function T(aN){var aM=new CustomEvent("inputPositionChanged",{detail:aN});
document.dispatchEvent(aM);
h(aN.uiFlags);
}function an(aN){var aM=new CustomEvent("textViewLocationChanged",{detail:aN});
document.dispatchEvent(aM);
h(aN.uiFlags);
}function E(aN){var aM=new CustomEvent("dbSchemeReceived",{detail:aN});
document.dispatchEvent(aM);
}function ai(aN){var aM=new CustomEvent("componentCursorChanged",{detail:aN});
document.dispatchEvent(aM);
}function I(aN){var aM=new CustomEvent("documentTargetsReceived",{detail:{targets:aN.targets}});
document.dispatchEvent(aM);
}function V(aN){var aM=new CustomEvent("txItemReceived",{detail:aN});
document.dispatchEvent(aM);
switch(aN.itemType){case b.ItemType.FieldAdapter:aM=new CustomEvent("fieldDataReceived",{detail:aN.item});
document.dispatchEvent(aM);
break;
case b.ItemType.TextField:aM=new CustomEvent("textFieldDataReceived",{detail:aN});
document.dispatchEvent(aM);
break;
}}function F(aN){var aM=new CustomEvent("dialogResourcesReceived",{detail:aN});
document.dispatchEvent(aM);
}function ah(aN){var aM=new CustomEvent("appModeChanged",{detail:aN});
document.dispatchEvent(aM);
}function ak(aM){switch(aM.dialogCategory){case b.DialogCategory.HTMLDialog:az(aM);
break;
case b.DialogCategory.TXDialog:if(!b.txDialog){b.txDialog=new b.TxDialog(aM.initialImage,aM.caption,aM.dialogType);
b.txDialog.showModal();
}else{b.txDialog.showChildDialog(aM.initialImage,aM.caption);
}break;
}}function az(aN){var aM;
switch(aN.dialogType){case b.HTMLDialogType.InputBoxDialog:aA(aN);
break;
case b.HTMLDialogType.MaxPreviewRowsDialog:aB(aN);
break;
case b.HTMLDialogType.ChildDialog:if(b.txDialog){b.txDialog.showChildDialog(aN.initialImage,aN.caption);
}else{b.jQueryAlert("Error Message","Can not open child dialog without having any TXTextControl dialog open.");
}break;
case b.HTMLDialogType.EditDataRelationsDialog:ax(aN.relations);
break;
case b.HTMLDialogType.EditMergeBlocksDialog:ay(aN.blocks);
break;
case b.HTMLDialogType.OptionsDialog:aC(aN.options);
break;
case b.HTMLDialogType.ChartSettingsDialog:aw(aN.properties);
break;
case b.HTMLDialogType.InsertMergeBlockDialog:aM=new b.InsertMergeBlockDialog(aN.name,b.dbScheme);
aM.showModal();
break;
case b.HTMLDialogType.SaveDocumentAsDialog:aD(aN.fileName,aN.streamType);
break;
case b.HTMLDialogType.SpellCheckDialog:aM=new b.SpellCheckDialog(aN);
aM.showModal();
break;
case b.HTMLDialogType.LoadDictionaryDialog:aM=new b.LoadDictionaryDialog(aN);
aM.showModal();
break;
}}function K(aN){var aM=new CustomEvent("fieldDisplayModeChanged",{detail:aN});
document.dispatchEvent(aM);
}function aj(aN){var aM=new CustomEvent("ribbonButtonStateMessageReceived",{detail:aN});
document.dispatchEvent(aM);
}function ap(aN){var aM=new CustomEvent("waitDialogMessageReceived",{detail:aN});
document.dispatchEvent(aM);
}function af(aN){var aM=new CustomEvent("masterTableSelected",{detail:aN});
document.dispatchEvent(aM);
}function Y(aN){var aM=new CustomEvent("mergeFieldHighlightingChanged",{detail:aN});
document.dispatchEvent(aM);
}function X(aN){var aM=new CustomEvent("mergeBlockHighlightingChanged",{detail:aN});
document.dispatchEvent(aM);
}function A(aN){var aO=aN.target;
switch(aO){case b.ComponentType.TxDialog:var aM=new CustomEvent("dialogCaretStateChanged",{detail:aN});
document.dispatchEvent(aM);
break;
case b.ComponentType.TextView:var aM=new CustomEvent("textViewCaretStateChanged",{detail:aN});
document.dispatchEvent(aM);
break;
}}function B(aN){var aM=new CustomEvent("txDialogClosed",{detail:aN});
document.dispatchEvent(aM);
}function R(aN){var aM=new CustomEvent("imageRegionsUpdated",{detail:aN});
document.dispatchEvent(aM);
}function aa(aN){var aM=new CustomEvent("viewContentMovedAndUpdated",{detail:aN});
document.dispatchEvent(aM);
}function D(aN){var aM=new CustomEvent("contextMenuDataReceived",{detail:aN});
document.dispatchEvent(aM);
}function C(aM){var aN=aM.text;
switch(aM.type){case b.ConsoleLogType.Log:console.log(aN);
break;
case b.ConsoleLogType.Debug:console.debug(aN);
break;
case b.ConsoleLogType.Info:console.info(aN);
break;
case b.ConsoleLogType.Warn:console.warn(aN);
break;
case b.ConsoleLogType.Error:console.error(aN);
break;
}}function J(aN){var aM=new CustomEvent("drawShapeMessageReceived",{detail:aN});
document.dispatchEvent(aM);
}function P(aN){var aM=new CustomEvent("hyperlinkClicked",{detail:aN.link});
document.dispatchEvent(aM);
}function ao(){}function al(aM){var aN=aM.resources;
switch(aM.type){case aF.VariousMessages:b.variousMessages=aN;
break;
case aF.RibbonStrings:b.ribbonStringResources=aN;
b.initRibbon();
break;
}}function ad(aM){if(aM.isSpellCheckingEnabled){a("#btnSpelling").removeClass("ui-state-disabled");
...
}if(aM.isLangDetectEnabled){a("#tglBtnEnableLangDetect").addClass("ribbon-button-selected");
}else{a("#tglBtnEnableLangDetect").removeClass("ribbon-button-selected");
}if(aM.isHyphenationEnabled){a("#tglBtnEnableHyphenation").addClass("ribbon-button-selected");
}else{a("#tglBtnEnableHyphenation").removeClass("ribbon-button-selected");
}}function U(aN){var aM=null;
switch(aN.itemType){case b.ItemType.HeaderFooter:switch(aN.activationState){case b.ItemActivationState.Activated:aM=new CustomEvent("headerFooterActivated");
break;
case b.ItemActivationState.Deactivated:aM=new CustomEvent("headerFooterDeactivated");
break;
}break;
}if(aM){document.dispatchEvent(aM);
}}function O(aN){switch(aN.dialogType){case b.HTMLDialogType.SpellCheckDialog:var aM=new CustomEvent("spellCheckDialogStateUpdated",{detail:aN.state});
document.dispatchEvent(aM);
break;
}}function W(aN){var aM;
switch(aN.menuType){case b.MenuType.InsertShapes:b.insertShapesMenuContent=aN.menuItems;
break;
}}function aq(aM){var aN="";
switch(aM.reason){case aI.CouldNotConnectToService:aN="The web server could not connect to the\r\nTX Text Control service for unknown reasons.";
break;
case aI.ServiceNotInstalled:aN="The web server could not connect to the TX Text Control service\r\nbecause the service is not installed.";
break;
case aI.ServiceNotRunning:aN="The web server could not connect to the TX Text Control service\r\nbecause the service is not running.";
break;
}if(aN){b.variousMessages.webSocketErrMsg.text=aN;
}console.error(aM.excType+": "+aM.excMsg);
}function ag(aM){switch(aM.eventType){case b.ServerEventType.DocumentLoaded:n(aM.eventData);
break;
}}function n(aN){var aM={fileName:aN.FileName};
b.fireEvent(b.EventName.documentLoaded,aM);
}function G(aQ){var aN=b.saveDocumentCallbacks;
b.saveDocumentCallbacks=[];
if(aQ.excMsg){throw Error(aQ.excMsg);
}for(var aP=0;
aP<aN.length;
++aP){var aM=aN[aP];
try{aM({data:aQ.data,streamType:aQ.streamType});
}catch(aO){}}}function ae(aQ){var aN=b.saveSelectionCallbacks;
...
++aP){var aM=aN[aP];
try{aM({data:aQ.data,streamType:aQ.streamType});
}catch(aO){}}}b.fileSaveQuestion=function(aO){aO=aO||function(){};
if(!b.saveDocumentQuestionEnabled){aO();
return;
}var aS=b.txDialog?b.txDialog.getActiveViewer():b.textViewer;
var aN=b.variousMessages.fileSaveQuestionDlg.caption;
var aR=b.variousMessages.fileSaveQuestionDlg.text;
var aT=b.variousMessages.fileSaveQuestionDlg.yesBtnTxt;
var aP=b.variousMessages.fileSaveQuestionDlg.noBtnTxt;
var aM=b.variousMessages.fileSaveQuestionDlg.cancelBtnTxt;
b.documentSavedEventHandlers=[];
var aQ=function(){b.sendCommand(b.Command.Save);
b.documentSavedEventHandlers.push(aO);
};
a('<div style="font-size: 10pt; min-width: 250px">'+aR+"</div>").dialog({modal:true,resizable:false,title:aN,width:"auto",height:130,buttons:{Yes:{text:aT,click:function(){a(this).dialog("destroy").remove();
aQ();
if(aS&&!b.isTouchDevice){aS.focus();
}}},No:{text:aP,click:function(){a(this).dialog("destroy").remove();
aO();
if(aS&&!b.isTouchDevice){aS.focus();
}}},Cancel:{text:aM,click:function(){a(this).dialog("destroy").remove();
if(aS&&!b.isTouchDevice){aS.focus();
}}}},close:function(){a(this).dialog("destroy").remove();
if(aS&&!b.isTouchDevice){aS.focus();
}}});
};
function aA(aM){var aN=new b.InputBoxDialog(aM.title,aM.text);
aN.showModal();
}function aB(aM){var aN=new b.MaxPreviewRowsDialog(aM.value);
aN.showModal();
}function ax(aN){var aM=new b.EditDataRelationsDialog(aN);
aM.showModal();
}function ay(aM){var aN=new b.EditMergeBlocksDialog(aM);
aN.showModal();
}function aC(aN){var aM=new b.OptionsDialog(aN);
aM.showModal();
}function aw(aN){var aM=new b.ChartSettingsDialog(aN);
aM.showModal();
}function aD(aN,aO){var aM=new b.SaveDocumentAsDialog(aN,aO);
aM.showModal();
}function o(aM){if(b.isTouchDevice){return;
}var aN=document.getElementById("txTemplateDesignerContainer");
if((aM==aN)||(aN.contains(aM))){if((aM.tagName!=="SELECT")&&(aM.tagName!=="INPUT")){b.textViewer.focus();
}}}function u(aM){var aN=document.getElementsByClassName("scrollbar-wrapper");
for(var aO=0;
aO<aN.length;
++aO){if(aN[aO].contains(aM)){o(aM);
break;
}}}function j(aN){try{u(aN.target);
t.Left=(aN.which==1);
t.Middle=(aN.which==2);
t.Right=(aN.which==3);
}catch(aM){}}function k(aN){try{if(aN.which==1){t.Left=false;
}if(aN.which==2){t.Middle=false;
}if(aN.which==3){t.Right=false;
}if(!t.Left&&!t.Middle&&!t.Right){b.mouseOwner=null;
}}catch(aM){}}function i(aN){try{o(aN.target);
}catch(aM){}}function y(){}function l(){for(var aN=0;
aN<b.documentSavedEventHandlers.length;
++aN){var aM=b.documentSavedEventHandlers[aN];
aM();
}b.documentSavedEventHandlers=[];
}function z(aM){aM.preventDefault();
}b.webSocketDisconnect=function(){if(!b.socket){return;
}if(b.isSocketConnected()){b.socket.removeEventListener("error",aJ);
b.socket.removeEventListener("close",aH);
b.socket.close();
}b.socket=null;
};
function v(aM){try{b.sendCommand(b.Command.Close);
b.webSocketDisconnect();
}catch(aM){}}b.jQueryAlert=function(aM,aN,aO){aM=aM||"";
aN=aN||"";
aN=aN.replace(/\r?\n/g,"<br />");
aO=typeof aO!=="undefined"?aO:null;
a('<div style="font-size: 10pt; min-width: 250px">'+aN+"</div>").dialog({modal:true,resizable:false,title:aM,width:"auto",buttons:{OK:function(){a(this).dialog("destroy").remove();
if(aO&&!b.isTouchDevice){aO.focus();
}},},close:function(){a(this).dialog("destroy").remove();
if(aO&&!b.isTouchDevice){aO.focus();
}}});
};
...
!Array.prototype.contains&&(Array.prototype.contains=function(a){return this.find(a)!=null;
});
